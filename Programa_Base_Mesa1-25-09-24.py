...
#Valor sensor derecho centro:17
#Valor sensor derecho borde:48
#Valor sensor frontal centro:8
#Valor sensor fronta borde:27

#umbrales de negro
centro_negro = 16
cenntro_negro_frente= 8
borde_negro = 48
...
def seguidor_de_linea():
    ...
    if tiempo_actual > 5000:
        ...
        while True:
            ...
            if color_sensor_r.reflection()<(borde_negro+5):
                parar_motores()
                movermotores(-80,-80,0.2)
                buscar_linea_frontal_der
                ultima_deteccion.reset()
                break
            elif color_sensor_l.reflection()<(borde_negro+5):
                parar_motores()
                movermotores(-80,-80,0.2)
                buscar_linea_frontal_izq()
                ultima_deteccion.reset()
                break
    ...
    # Esperar 1 ms para la próxima iteración
    wait(1)
...
def buscar_linea_frontal_der():
    parar_motores()
    hub.display.text("BLF")
    hub.imu.reset_heading(0)
    while hub.imu.heading() < 45:
        mover_motores_indefinido(100,-100)
    parar_motores()

    while color_sensor_f.reflection() > (cenntro_negro_frente+2):
        mover_motores_indefinido(-100,100)
    parar_motores()

def buscar_linea_frontal_izq():
    parar_motores()
    hub.display.text("BLF")
    hub.imu.reset_heading(0)
    while hub.imu.heading() < 45:
