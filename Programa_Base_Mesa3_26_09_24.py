...
#Aqui se calibran los colores a utilizar en la pista en modo HSV 
Color.BLACK = Color(h=180, s=20, v=27)
Color.GREEN = Color(h=166, s=68, v=34)

#lista de colores a utilizar con el sensor, si no se utiliza algun color creo que es mejor no ponerlo
colores = (Color.BLACK, Color.GREEN,Color.NONE,Color.WHITE)

#aqui le digo a los sensores que van a trabajar con la lista que se creo anteriormente
...
def buscar_linea_izquierda():
    ...
    while color_sensor_l.reflection() > (centro_negro+5)  :
        mover_motores_indefinido(60,-60) #giro a la izquierda para acomodarme a la linea con el sensor izquierdo
    parar_motores()

    while color_sensor_r.reflection() > (centro_negro+5):
        mover_motores_indefinido(-60,60) #giro a la derecha para acomodarme a la linea con el sensor derecho
    parar_motores()
...
def main():
    while True:
        ...
        seguidor_de_linea()
        #rescate()
        #print(color_sensor_l.reflection(),color_sensor_r.reflection(),color_sensor_f.reflection())
