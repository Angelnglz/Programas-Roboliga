...
def seguidor_de_linea_comun():
    # Comprobar si algún sensor detecta la línea negra
    if light_l < borde_negro:
        motor_izquierdo.dc(-45)
    else:
        motor_izquierdo.dc(30)
    if light_r < borde_negro:      
        motor_derecho.dc(-45)
    else:
        motor_derecho.dc(30)
...
def obstaculo():
    ...
    if -1<distance<48: 
        ...
        if -1<distance_sensor.distance()<50: #confirmo si sigo detectando 
            ...
            movermotores(-100,-100,0.1) #retrocedo un poco para alejarme 
            ...
            while color_sensor_l.reflection() > centro_negro:
                mover_motores_indefinido(425,185) #giro con direccion derecha hasta encontrar el centro de la linea con el sensor izquierdo
            parar_motores()
